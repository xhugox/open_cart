...
if(!$this->getProductadvoptionbycustomergroup($this->config->get('config_customer_group_id'),$product_option_value['product_option_value_id']))
				$product_option_value_data[] = array(
			...
			'default_select'          => ($this->config->get('module_advancedoption_default_status') ? $product_option_value['default_status'] : ''),
			'sku'					  => ($this->config->get('module_advancedoption_sku') ? $this->getproductoptionvaluedata('sku',$product_option_value['product_option_value_id']) : ''),
			'image'					  => ($this->config->get('module_advancedoption_image') ? $this->getproductoptionvaluedata('image',$product_option_value['product_option_value_id']) : $product_option_value['image']),
		...
		return $product_option_data;
			...
			public function getProductadvoptionbycustomergroup($customer_group_id, $product_option_value_id){
				$this->createtable();
				if($this->config->get('module_advancedoption_customer_group') && $this->config->get('module_advancedoption_status')){
					$query = $this->db->query("SELECT * FROM ".DB_PREFIX."product_option_value_data WHERE product_option_value_id = '" . (int)$product_option_value_id . "' AND 	customer_group_ids LIKE '%".$customer_group_id."%'");
					if($query->row){
						return true;
					}else{
						return false;
			...
			public function getimagesdata($product_id){
				$this->createtable();
				$product_option_value_data = array();
				if($this->config->get('module_advancedoption_image')){
					
					
					$product_option_value_query =  $this->db->query("SELECT * FROM ".DB_PREFIX."product_option_value_data WHERE product_id = '".(int)$product_id."'");
					...
					foreach($product_option_query->rows as $product_option){
						$product_option_value_query = $this->db->query("SELECT ov.image,pov.product_option_value_id  FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_id = '" . (int)$product_id . "' AND pov.product_option_id = '" . (int)$product_option['product_option_id'] . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ov.sort_order");
