...
class ModelCatalogProduct extends Model {
				...
				public function addoptiondata($product_option_value,$product_option_value_id,$product_id){
					 if($this->config->get('module_advancedoption_status')){
						$this->db->query("DELETE FROM " . DB_PREFIX . "product_option_value_data WHERE product_option_value_id = '" . (int)$product_option_value_id . "'");
						
						$sql = "INSERT INTO " . DB_PREFIX . "product_option_value_data SET product_option_value_id = '" . (int)$product_option_value_id . "', product_id = '".(int)$product_id."'";
						
						if(isset($product_option_value['model'])){
							$sql .= ", model = '" . 	$this->db->escape($product_option_value['model']) . "'";
						}
						
						if(isset($product_option_value['image'])){
							$sql .= ", image = '" . 	$this->db->escape($product_option_value['image']) . "'";
						}
						
						if(isset($product_option_value['sku'])){
							$sql .= ", sku = '" . 	$this->db->escape($product_option_value['sku']) . "'";
						}
						
						if(isset($product_option_value['upc'])){
							$sql .= ", upc = '" . 	$this->db->escape($product_option_value['upc']) . "'";
						}
						
						if(isset($product_option_value['ean'])){
							$sql .= ", ean = '" . 	$this->db->escape($product_option_value['ean']) . "'";
						}
						
						if(isset($product_option_value['jan'])){
							$sql .= ", jan = '" . 	$this->db->escape($product_option_value['jan']) . "'";
						}
						
						if(isset($product_option_value['customer_group_id'])){
							$sql .= ", customer_group_ids = '" . 	json_encode($product_option_value['customer_group_id']) . "'";
						}
						
						$this->db->query($sql);
						if($this->config->get('module_advancedoption_default_status')){
						$this->db->query("UPDATE ".DB_PREFIX."product_option_value SET default_status = '".(int)(isset($product_option_value['default_status']) ? $product_option_value['default_status'] : 0)."' WHERE product_option_value_id = '".(int)$product_option_value_id."' AND product_id = '".(int)$product_id."'");
						}
					 }
				}
		...
		if (isset($data['product_option'])) {
			...
			//EOF Related Options
				...
				if(empty($product_option_value_id)){
					$product_option_value_id = $this->db->getLastId();	
					$this->addoptiondata($product_option_value,$product_option_value_id,$product_id);
				}else{
					$this->addoptiondata($product_option_value,$product_option_value_id,$product_id);
				}
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			foreach ($product_option_value_query->rows as $product_option_value) {
				$product_option_value_data[] = array(
					...
					'customer_group_ids'	  => $this->getproductoptionvaluedata('customer_group_ids',$product_option_value['product_option_value_id']),
					'default_status'          => $product_option_value['default_status'],
					'model'					  => $this->getproductoptionvaluedata('model',$product_option_value['product_option_value_id']),
					'sku'					  => $this->getproductoptionvaluedata('sku',$product_option_value['product_option_value_id']),
					'image'					  => $this->getproductoptionvaluedata('image',$product_option_value['product_option_value_id']),
					'upc'					  => $this->getproductoptionvaluedata('upc',$product_option_value['product_option_value_id']),
					'jan'					  => $this->getproductoptionvaluedata('jan',$product_option_value['product_option_value_id']),
					'ean'					  => $this->getproductoptionvaluedata('ean',$product_option_value['product_option_value_id']),
